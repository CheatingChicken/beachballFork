...
BeachBall.audio_Bell = new Audio("https://www.soundjay.com/misc/bell-ring-01.wav");
BeachBall.audio_Bell.volume = 1;
BeachBall.audio_Chime = new Audio("https://www.soundjay.com/misc/small-bell-ring-01a.wav");
BeachBall.audio_Chime.volume = 1;
BeachBall.RKAlertFrequency = 8;
if (Molpy.Got("Kitnip") == 1) {
    BeachBall.RKAlertFrequency = 10;
}
...
BeachBall.PuzzleConstructor = function (name) {
    ...
    this.ParseClaim = function (claimText) {
        ...
        if ((i > 0 && k < 0) || j * k > 0) claim.value = true;
    ...
    this.PopulateStatements = function () {
        var i = 0;
        var j =
            this.puzzleString.indexOf("]<br>") > 0
                ? this.puzzleString.indexOf("]<br>") + 5
                : this.puzzleString.indexOf(">") + 1;
    ...
    this.EvaluateKnownStatements = function () {
        for (i in this.statement) {
            ...
            // A: A is true AND A is false; A must be false.
                ...
                me.condition == "and" &&
                me.claim[0].name == me.name &&
                me.claim[1].name == me.name &&
                me.claim[0].value != me.claim[1].value
            ) {
                ...
                for (k in me.claim) {
                    ...
                    if (me.claim[k].name == this.statement[index].name) {
                        if (typeof me.condition == "undefined") {
                            ...
                            if (
                                typeof me.claim[k].result == "boolean" &&
                                me.claim[m].name == me.name
                            ) {
        ...
        for (var i = 0; i < this.statement.length; i++) {
            for (j in this.statement[i].claim) {
                ...
                if (typeof me != "undefined") {
                    ...
                    console.log(
                        "Error with " + this.statement[i].name + " i: " + i + " and j: " + j,
                    );
                }
            }
        }
...
BeachBall.CagedAutoClick = function () {
    ...
    if (
        me.status > 0 &&
        Molpy.Got("LogiPuzzle") > 1 &&
        !Molpy.PuzzleGens["caged"].active &&
        !BeachBall.cagedTimeout
    ) {
        ...
        // Buy Maximum Puzzles, or Singles if Max is less than 10
            ...
            if (
                Molpy.PokeBar() >= 11 &&
                Molpy.Level("LogiPuzzle") >= Molpy.PokeBar() &&
                tens &&
                Molpy.Has("GlassBlocks", costMulti)
            ) {
    ...
    if (
        Molpy.PuzzleGens["caged"].active &&
        (me.status == 1 || me.status == 2 || meLC.status == 1) &&
        Molpy.PuzzleGens["caged"].guess[0] == "No Guess"
    ) {
        ...
        if (Molpy.Got("LogiPuzzle") > 1) {
            ...
            BeachBall.cagedTimer = setTimeout(function () {
                BeachBall.cagedTimeout = false;
            }, BeachBall.cagedTimeoutLength);
        }
    }
};
        ...
        if (Molpy.Boosts["MHP"].unlocked) {
            ...
            if (!Molpy.Got("MHP")) {
                var sp = Math.floor(
                    Molpy.priceFactor *
                        100 *
                        Math.pow(2, Math.max(1, Molpy.Boosts["MHP"].power - 9)),
                    1,
                );
                var gp = 0;
                if (Molpy.IsEnabled("HoM")) {
                    gp = Math.floor(
                        Molpy.priceFactor *
                            100 *
                            Math.pow(2, Math.max(1, Molpy.Boosts["MHP"].power - 15)),
                        1,
                    );
                }
...
BeachBall.RiftAutoClick = function () {
    if (BeachBall.Settings["RiftAutoClick"].status == 0) return;
    ...
    switch (parseInt(BeachBall.Settings["RiftAutoClick"].status)) {
        case 1: // farm crystals
            ...
            if (
                !(
                    Molpy.Boosts["Time Lord"] &&
                    Molpy.Boosts["Time Lord"].bought &&
                    Molpy.Boosts["Time Lord"].power
                )
            )
                return;
...
BeachBall.GetBeachState = function () {
    ...
    if (!Molpy.ninjad) {
        if (Molpy.npbONG) stateClass = "beachstreakextend";
...
BeachBall.Ninja = function () {
    ...
    if (Molpy.ninjad == 0) {
        ...
        if (Molpy.npbONG != 0) {
            BeachBall.incoming_ONG = 0;
            if (
                BeachBall.Settings["BeachAutoClick"].status > 0 &&
                Molpy.Got("Temporal Rift") == 0
            ) {
                Molpy.ClickBeach();
...
BeachBall.PlayRKAlert = function () {
    ...
    if (
        Math.floor(BeachBall.RKTimer % BeachBall.RKAlertFrequency) == 0 &&
        BeachBall.RKPlayAudio == 1
    ) {
...
BeachBall.RedundaKitty = function () {
    ...
    if (Molpy.Redacted.location > 0) {
        ...
        if (
            BeachBall.Settings["AudioAlerts"].status == 1 ||
            BeachBall.Settings["AudioAlerts"].status == 4
        ) {
            BeachBall.PlayRKAlert();
        }
        // If LC Audio Alert Enabled and LC is available, Play Alert
        else if (
            BeachBall.Settings["AudioAlerts"].status == 2 &&
            Molpy.Redacted.DrawType[Molpy.Redacted.DrawType.length - 1] == "hide2"
        ) {
            BeachBall.PlayRKAlert();
        }
    }
...
BeachBall.ChooseAutoclick = function () {
    ...
    if (speed === null) {
        ...
        Molpy.Notify(
            "Favorite asignation for " +
                Molpy.activeLayout.faves[selectedFave].boost.name +
                " has been removed",
            1,
        );
    ...
    var period = speed_el[2] == "s" ? speed_el[1] * 1000 : 1000 / speed_el[1];
...
BeachBall.ToggleAutoclickFav = function (fav, shown) {
    ...
    if (shown)
        Molpy.Notify(
            "Autoclick Favorite " +
                Molpy.activeLayout.faves[fav].boost.name +
                " toggled : " +
                (me.timer ? "activated, " + me.speed : "disabled"),
            1,
        );
};
        ...
        if (me && me.period && $("#faveHeader" + fav + " h1"))
            if ($("#faveHeader" + fav + " h1 .BB_autoclick").length == 0) {
                if ($("#faveHeader" + fav + " h1").length > 0)
                    $("#faveHeader" + fav + " h1")[0].innerHTML =
                        $("#faveHeader" + fav + " h1")[0].innerHTML +
                        "<a class='BB_autoclick' onclick='BeachBall.ToggleAutoclickFav(\"" +
                        fav +
                        "\",true)' " +
                        (me.timer ? "" : "style='text-decoration:line-through' ") +
                        ">[ " +
                        me.speed +
                        " ]</a>";
            } else {
                $("#faveHeader" + fav + " h1 .BB_autoclick")
                    .first()
                    .css("text-decoration", me.timer ? "" : "line-through");
            }
    }
};

BeachBall.LoadAutoclickFav = function () {
    BeachBall.FavsAutoclick = localStorage["BB.FavsAutoclick"]
        ? JSON.parse(localStorage["BB.FavsAutoclick"])
        : {};
    for (fav in BeachBall.FavsAutoclick) {
        var me = BeachBall.FavsAutoclick[fav];
        if (me && me.timer) {
            // if there was an active timer when the save occured
            me.timer = 0;
...
BeachBall.CreateMenu = function () {
    if (window.location.pathname == "/classic.html" && g("sectionOptions") != null) {
        // patch for option height in classic
    ...
    new Molpy.Option({
        ...
        breakafter: true,
        text: function () {
            return '<h4 style"font-size:75%">v ' + BeachBall.version + "</div>";
        },
    });
    ...
    for (var i = 0; i < BeachBall.AllOptions.length; i++) {
        ...
        new Molpy.Option({
            ...
            onchange: (function (_option) {
                return function () {
                    BeachBall.SwitchStatus(_option);
                };
            })(option),
            text: (function (_option) {
                return function () {
                    return BeachBall.DisplayDescription(_option, "desc");
                };
            })(option),
        });
    }
    ...
    $("#faveControls").append(
        '<div id="autoclickFave" class="minifloatbox"><a onclick="BeachBall.ChooseAutoclick()">AutoClick</a></div>',
    );
};
...
BeachBall.DisplayDescription = function (option, type) {
    ...
    if (option == "ToolFactory") {
        if (Molpy.Boosts["TF"].bought == 1) {
            ...
            description =
                'Amount to load : <a onclick="BeachBall.SwitchSetting(\'ToolFactory\')" title="Click to change me !">' +
                me.setting +
                " chips</a><br/>Click option title to load.";
        } else {
            title = '<h4>Tool Factory Locked</h4><div id="ToolFactoryDesc"></div>';
            description =
                'Factory locked... <a onclick="BeachBall.CheckToolFactory()">Check Again!!</a>';
        }
    }
    ...
    if (type == "desc") return description;
...
BeachBall.LoadDefaultSetting = function (option, key) {
    if (option == "AudioAlerts") {
        ...
        if (key == "desc") {
            return ["Off", "RK Only", "LC Only", "ONG Only", "All Alerts"];
        }
    } else if (option == "BeachAutoClick") {
        if (key == "title") {
            return "Beach AutoClick";
        }
        if (key == "status") {
            return 1;
        }
        ...
        if (key == "msg") {
            return "Please enter your desired clicking rate per second (1 - 20):";
        }
        if (key == "desc") {
            return [
                "Off",
                "Keep Ninja",
                "On: <a onclick=\"BeachBall.SwitchSetting('BeachAutoClick')\">" +
                    BeachBall.Settings[option].setting +
                    " cps</a>",
                "On: <a onclick=\"BeachBall.SwitchSetting('BeachAutoClick')\">" +
                    BeachBall.Settings[option].setting +
                    " cps</a><br/>(with ninja ritual)",
            ];
        }
    } else if (option == "CagedAutoClick") {
        if (key == "title") {
            return "Caged Logicat AutoClick";
        }
        if (key == "status") {
            return 0;
        }
        ...
        if (key == "desc") {
            return ["Off", "Solve Single", "Solve Max", "Get Bonemeal"];
        }
    } else if (option == "LCSolver") {
        if (key == "title") {
            return "Logicat Solver";
        }
        if (key == "status") {
            return 0;
        }
        ...
        if (key == "maxSetting") {
            return Molpy.NPlength;
        }
        if (key == "msg") {
            return "Please enter your desired refresh rate in milliseconds (500 - 3600):";
        }
        if (key == "desc") {
            return [BeachBall.Settings[option].setting];
        }
    } else if (option == "RKAutoClick") {
        if (key == "title") {
            return "Redundakitty AutoClick";
        }
        if (key == "status") {
            return 0;
        }
        ...
        if (key == "msg") {
            return "Tool Factory Loading:";
        }
        if (key == "desc") {
            return ["Off", BeachBall.Settings[option].setting];
        }
    } else if (option == "RiftAutoClick") {
        if (key == "title") {
            return "Rift Autoclick";
        }
        if (key == "status") {
            return 0;
        }
        ...
        if (key == "desc") {
            return ["Off", "On - Flux Cristal", "On - ONG"];
        }
    } else {
        Molpy.Notify(
            BeachBall.Settings[option] + " setting not found. Please contact developer.",
            1,
        );
        return -1;
    }
...
BeachBall.SwitchStatus = function (option) {
    ...
    if (option == "ToolFactory") BeachBall.LoadToolFactory();
...
BeachBall.StartProgram = function () {
    ...
    Molpy.Notify(
        "BeachBall version " +
            BeachBall.version +
            " loaded for SandCastle Builder version " +
            BeachBall.SCBversion,
        1,
    );
